...
(function () {
  ...
  function ProgressBar({ value, color = t.accent }) {
    return (
      <div style={{ height: 4, background: t.surface, borderRadius: 6, overflow: 'hidden' }}>
        <div style={{ width: `${value * 100}%`, height: '100%', background: color, transition: 'width .3s ease' }} />
      </div>
    );
  }
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: t.sans, fontSize: 17, fontWeight: 500, color: t.text, lineHeight: 1.2 }}>
              {team.n}
            </div>
            <div style={{
              ...
              <span style={{ width: 8, height: 8, borderRadius: 99, background: team.k }} />
              {team.c}
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontFamily: t.mono, fontSize: 18, color: complete ? t.accent : t.text, fontWeight: 600, lineHeight: 1 }}>
              {String(owned).padStart(2, '0')}
              <span style={{ color: t.textDim, fontSize: 13, fontWeight: 500 }}>/{PER_TEAM}</span>
            </div>
            {complete && (
    ...
    return (
      <div style={{ marginTop: 24, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: t.sans, fontSize: 13, color: t.textDim, textTransform: 'uppercase', letterSpacing: 0.8 }}>
              Grupo {death && <span style={{ color: t.red }}>· da Morte</span>}
            </div>
          </div>
          <div style={{ fontFamily: t.mono, fontSize: 14, color: t.text, fontWeight: 500 }}>
            {owned}<span style={{ color: t.textDim }}>/{total}</span>
          </div>
        </div>
      </div>
    );
  ...
  function SpecialSection({ title, kicker, ids, collected, handlers }) {
    let owned = 0;
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div>
            ...
            <div style={{ fontFamily: t.sans, fontSize: 17, lineHeight: 1.3, color: t.text, marginTop: 2, fontWeight: 500 }}>
              {title}
            </div>
          </div>
          <div style={{ fontFamily: t.mono, fontSize: 16, color: t.text, fontWeight: 500 }}>
            {owned}<span style={{ color: t.textDim, fontSize: 13 }}>/{ids.length}</span>
          </div>
        </div>
  ...
  function AnthropicApp() {
    ...
    return (
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: t.bg, fontFamily: t.sans, color: t.text, overflow: 'hidden' }}>
        {/* header */}
        <div style={{ padding: '20px 20px 16px', borderBottom: `1px solid ${t.line}` }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
            <div>
              ...
              <h1 style={{ fontFamily: t.sans, fontSize: 24, lineHeight: 1.2, margin: '4px 0 0', fontWeight: 500, color: t.text }}>
                Álbum de figurinhas
              </h1>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontFamily: t.mono, fontSize: 28, lineHeight: 1, color: t.text, fontWeight: 500 }}>
                {owned}
                <span style={{ color: t.textDim, fontSize: 16 }}>/{TOTAL}</span>
              </div>
              <div style={{ fontFamily: t.sans, fontSize: 12, color: t.textDim, marginTop: 4 }}>
          ...
          {[['album', 'Álbum'], ['troca', 'Trocas']].map(([k, label]) => (
            <button key={k} onClick={() => setTab(k)}
              style={{
                ...
                fontFamily: t.sans, fontSize: 14, fontWeight: tab === k ? 500 : 400,
                color: tab === k ? t.text : t.textDim,
                borderBottom: tab === k ? `2px solid ${t.accent}` : '2px solid transparent',
                marginBottom: -1, cursor: 'pointer', transition: 'color .2s',
              }}>
              {label}
              {k === 'troca' && dups.length > 0 && (
                <span style={{
                  marginLeft: 6, fontFamily: t.mono, fontSize: 11,
                  background: t.accentBg, color: t.accent, padding: '2px 6px', borderRadius: 6, fontWeight: 500,
                }}>{dups.length}</span>
              )}
            </button>
          ))}
        ...
        <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden' }}>
              ...
              <div style={{ position: 'relative', marginBottom: 10 }}>
                <input
                  ...
                  style={{
                    ...
                    fontFamily: t.sans, fontSize: 15, color: t.text, outline: 'none', transition: 'border .2s',
                  }}
                ...
                {query && (
                  <button onClick={() => setQuery('')}
                    style={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)',
                      background: 'none', border: 'none', color: t.textDim, fontSize: 20, padding: '4px 8px', cursor: 'pointer' }}>×</button>
                )}
              </div>

              <div style={{ fontFamily: t.sans, fontSize: 12, color: t.textSoft, textAlign: 'center', padding: '4px 0 12px' }}>
                Toque para marcar · Segure para adicionar repetida
              </div>

              {/* capa */}
              {!q && <SpecialSection title="Capa" kicker="Abertura · 1 figurinha" ids={['00']} collected={state.collected} handlers={handlers} />}
          ...
          {tab === 'troca' && (
            <TradePanel missing={missing} dups={dups} state={state} setState={setState} showToast={showToast} />
          )}
        </div>
        ...
        <div style={{
          ...
          background: t.surface2, color: t.text, padding: '10px 18px', borderRadius: 8,
          fontSize: 14, opacity: toast ? 1 : 0, transition: 'all .25s', pointerEvents: 'none',
          whiteSpace: 'nowrap', boxShadow: '0 8px 32px rgba(0,0,0,.4)', border: `1px solid ${t.line}`,
        }}>{toast}</div>
      </div>
    );
  }

  function TradePanel({ missing, dups, state, setState, showToast }) {
    const reset = () => {
    ...
    const Section = ({ kicker, title, count, items, empty }) => (
      <div style={{
        background: t.surface,
        borderRadius: 10,
        padding: 18,
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div>
            ...
            <div style={{ fontFamily: t.sans, fontSize: 18, lineHeight: 1.3, color: t.text, fontWeight: 500, marginTop: 2 }}>{title}</div>
          </div>
          <div style={{ fontFamily: t.mono, fontSize: 22, color: count > 0 ? t.accent : t.textDim, lineHeight: 1, fontWeight: 500 }}>
            {count}
          </div>
        </div>
        {items.length === 0 ? (
      ...
      <div style={{ paddingTop: 16, paddingBottom: 80 }}>
        ...
        <div style={{ padding: '20px', display: 'flex', justifyContent: 'center' }}>
          <button onClick={reset} style={{
            background: 'transparent', border: `1px solid ${t.line}`, color: t.textDim,
            padding: '10px 16px', borderRadius: 8, fontFamily: t.sans, fontSize: 13, cursor: 'pointer', transition: 'all .2s',
          }}
          onMouseEnter={e => { e.target.style.borderColor = t.accent; e.target.style.color = t.accent; }}
          onMouseLeave={e => { e.target.style.borderColor = t.line; e.target.style.color = t.textDim; }}
          >Zerar álbum</button>
        </div>
      </div>
    );
  ...
  window.AnthropicApp = AnthropicApp;
})();
